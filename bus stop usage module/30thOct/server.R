...
avgVolTrend <- mongo(collection = "avgVolTrend",db = databaseName, url = databaseUrl )
...
server <- function(input, output, session) {
  ...
  initialiseData <- function(bus){
    ...
    for (busStop in unlist(busStops)){
      ...
      dfAvgVol[[busStop]] <- avgVolTrend$find(query = toString(toJSON(list( stopId=busStop,busService = bus), auto_unbox = TRUE)))
      # if() {
      #   dfAvgVol[busStop]
      # }
      #print(dfAvgVol)
  ...
  updateData <- function(bus){
    ...
    for (busStop in unlist(busStops)){
      ...
      a <- avgVolTrend$find(query = toString(toJSON(list(startId = busStop, 
                                                      busService = bus),
                                                 auto_unbox = TRUE)))
